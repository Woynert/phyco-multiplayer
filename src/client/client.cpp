...
void Client::_bind_methods ()
{
	...
	ClassDB::add_property (
		"Client", PropertyInfo (Variant::INT, "port", PROPERTY_HINT_NONE),
		"set_port", "get_port");

	ClassDB::bind_method (D_METHOD ("get_address"), &Client::get_address);
	ClassDB::bind_method (
		D_METHOD ("set_address", "p_address"), &Client::set_address);
	ClassDB::add_property (
		"Client", PropertyInfo (Variant::STRING, "address"), "set_address",
		"get_address");
...
void Client::send_packages ()
{
	...
	peer->send (
		0, buf->get_data_array (), ENetPacketPeer::FLAG_UNRELIABLE_FRAGMENT);
}

// setters getters

int Client::get_port () const { return bind_port; }

void Client::set_port (const int p_port) { bind_port = p_port; }

String Client::get_address () const { return bind_address; }

void Client::set_address (const String p_address) { bind_address = p_address; }
