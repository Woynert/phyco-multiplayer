...
void Observer::_bind_methods ()
{
	...
	ADD_PROPERTY (
		PropertyInfo (
			Variant::INT, "type", PROPERTY_HINT_ENUM, "Player,RigidBody"),
		"set_type", "get_type");
...
void Observer::_ready () { add_to_group ("observer"); };

void Observer::set_type (Observer::ObjectType p_type) { type = p_type; }

Observer::ObjectType Observer::get_type () const { return type; }

void Observer::generate_id () { this->id = ++this->id_counter; }

void Observer::_set_id (int p_id) { this->id = p_id; }

void Observer::set_id (int p_id) { return; }

int Observer::get_id () const { return id; }
